...
const Post = ({getPost, deletePost, post: {post}, match, auth}) => {
    ...
    return (
        ...
        <div style={{float: "right"}}>
            <Button><FontAwesomeIcon icon={faArrowLeft} onClick={() => history.push("/forum")}></FontAwesomeIcon></Button>
            {!auth.loading && post.user === auth.user._id && 
            <Button variant="danger"><FontAwesomeIcon icon={faTrash} onClick={() => {deletePost(post._id); return history.push("/forum"); }} ></FontAwesomeIcon></Button>}
        </div>
        <p className="lead">
          por {post.name}
        </p>
